...
export default async function viewProject(req, res) {
  if (req.method === "GET") {
    ...
    if (!token) {
      res
        .status(200)
        .send({ access: false, loggedIn: false, message: "needs login" });
      return;
    }
    ...
    try {
      ...
      res
        .status(200)
        .json({ access: true, loggedIn: true, projects: projects });
    } catch (e) {
      res
        .status(500)
        .send({ access: false, loggedIn: true, message: e.message });
    }
    ...
    try {
      ...
      res.status(200).send({
        loggedIn: true,
        access: true,
        message: "Updated current status successfully",
      });
    } catch (e) {
      res
        .status(500)
