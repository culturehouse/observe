...
export default async function createProject(req, res) {
    ...
    if (!token) {
      res.status(200).send({created: false, loggedIn: false, message: "needs login"})
      return;
    }
    ...
    try {
        ...
        res.status(200).json({ access: true, loggedIn: true, created: true, new_heatmap: new_heatmap});
        ...
        res.status(500).send({ access:false, loggedIn: true, created: false, message: e.message });
    }
