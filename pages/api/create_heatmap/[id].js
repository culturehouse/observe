...
export default async function create_event(req, res) {
    ...
    if (!token) {
      res.status(200).send({validEvent: false, loggedIn: false, access: false,  message: "needs login"})
      return;
    }
    ...
    try {
        ...
        if (instances.length == 0) {
            ...
            if (instances[0].np_sub == response.decodedAccessToken.sub) {
                res.status(200).send({validEvent: true, loggedIn: true, access: true, data: instances})
            } else {
                res.status(200).send({validEvent: false, loggedIn: true, access: false, message: "Current user doesn't have access to the specified event instances"})
            }
        }
    } catch(e) {
