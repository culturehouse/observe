...
export default async function home(req, res) {
    ...
    if (!token) {
      res.status(200).send({dataFetched: false, loggedIn: false, message: "needs login"})
      return;
    }
    ...
    try {
        ...
        if (non_profit.length == 0) {
             res.status(200).send({dataFetched: false, loggedIn: true, message: "no non-profit"});
             return;
        }
        ...
        res.status(500).send({ dataFetched: false, dataFetched: false, message: e.message });
    }
