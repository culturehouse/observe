...
export default function Events({setshowCreateEvent, project, setLoggedIn, setCanAccess}) {
    ...
    // const [np_sub, setnp_sub] = useState("");
    // const [eCode, setECode] = useState("0000");
    ...
    const postToDatabase = () => {
      ...
      const response = fetch(`/api/create_event/${eventCode}`, {
        headers: {
        ...
        const res2 = fetch("/api/create_event/create_event", {
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
        ...
        // uploadFile(sketch, r.newEvent.id);
        // alert("New event [" + title + "] has been created.\n" + 
        // "Your unique event code is " + r.eCode);
  ...
  if (eventUploaded) {
    return (
      <div className={styles.successContainer}>
        {/* <Head>
          <title>
            Create Event | Observe
          </title>
        </Head> */}
        <div className={styles.border}>
          <div className={styles.closeButton} onClick={() => {window.location.reload()}}><FiX size={20}/></div>
          <div className={styles.title}>
              <h1 className={styles.maintitle}>{title}</h1>
            </div>
          <h4>Your unique event code: {eventCode}</h4>
          <p className={styles.italics}>Anyone with the code can add instances via the volunteer login</p>
          <h4 >Share your event code below</h4> 
          <div className={styles.flexRow}>
            <div className={styles.eventCode}>
              <h2>{eventCode}</h2>
              <div className={styles.buttonCode}>
                <div className={btnstyles.buttonEdit} onClick={() => {navigator.clipboard.writeText(eventCode); 
          ...
          <p onClick={() => {router.push(`/view_event?id=${eventUploadRes.newEvent.id}&projId=${project["id"]}&projName=${project["name"]}`)}} className={btnstyles.buttonCreate}>SEE NEW EVENT</p>
        </div>
      </div>
    )
  }

  if (postInProgress) {      // maybe add  && progress < 100 (so that image uploaded is part of loading)
    return (
    ...
    <div className={styles.dimmer}>
      <div className={styles.container}>
        ...
        <div className={styles.topHalfData}>
          ...
          <input className={styles.textInput} type="text" onChange={(e) => {setTitle(e.target.value); setEventCode(EventCode(title));}}/> <br/><br/> 
          <h4 className={styles.fieldTitle}>Start date</h4> 
          <input className={styles.textInput} type="date" onChange={(e) => setDate(e.target.value)} /> <br/><br/>
        </div>
        <div>
            <h4 className={styles.fieldTitle}>Location</h4>
            <input className={styles.textInput} type="text" onChange={(e) => setLocation(e.target.value)} /> <br/><br/>
            <h4 className={styles.fieldTitle}>Notes</h4> 
            <input className={styles.textInput} type="text" onChange={(e) => setNotes(e.target.value)} /> <br/><br/>
            <div className={styles.fileSelectContainer}>
              <label className={styles.fileContainer}>
                  {/* <input className={styles.fileInput} type="file" checked={coverImage} onChange={(e) => setCoverImage(!coverImage)}/> */}
                  <input className={styles.fileInput} type="file" checked={sketch} onChange={(e) => handleFileChange(e)}/>
                  <BsImage />
                  <p>{sketch ? sketch.name : "Add event sketch (.png)"}</p>
              </label>
              <p className={styles.x} onClick={() => setSketch(null)}><FiX /></p>
            </div>
            {/* <p>Works best with a 1:1 image ratio</p> */}
            <br></br> <br/>
        </div>
        <div className={styles.createProject}>
          <div className={btnstyles.buttonCreate} onClick={() => {postToDatabase()}} >CREATE EVENT</div>
