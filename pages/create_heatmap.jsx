...
export default function Home() {
  ...
  const createAggregateSNS = () => {
	if (instances.length == 0) {
		alert("It isn't possible to create a heatmap without having any instances that belong to the event")
		return;
	  ...
	  const res2 = fetch("/api/create_heatmap/create_heatmap", {
		...
		if (r.created) {
			...
			router.push(`/heatmap?id=${r.new_heatmap.id}&eventId=${router.query.eventId}&eventName=${router.query.eventName}&projId=${router.query.projId}&projName=${router.query.projName}`)
		} else {
			alert("Heatmap was not created, please try again")
		}
	  })
    // const res = await axios
	// 		.post('http://localhost:3001/create_aggregate', {
	// 			data: {
	// 				data: heatmapData,
	// 				instances: Array.from(selectedSet),
	// 				filterQuery: filters.map((f) => JSON.stringify(f)),
			...
			<div style={{ display: 'flex' }}>
				...
				>
					...
					<div className={styles.heatmapCon}>
						<DensityGraphCreateHeatmap data={heatmapData} width={350} height={350}/>
					</div>
				</div>
			</div>
			<div className={other_styles.instanceText}>
				<p style={{ display: 'flex', alignItems: 'center', paddingTop: '8px' }}>
					You have selected{' '}
					<span
						style={{
					...
					<div className={other_styles.bottom}>
							<div className={other_styles.buttonCreate} onClick={() => router.push(`/view_event?id=${router.query.eventId}&projId=${router.query.projId}&projName=${router.query.projName}`)}>
						CANCEL
					</div>
